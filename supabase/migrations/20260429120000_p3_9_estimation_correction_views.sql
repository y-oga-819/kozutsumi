...
--
-- References:
-- * docs/adr/0024-estimation-correction-by-category-median.md (中央値で算出)
-- * docs/adr/0025-estimation-correction-via-supabase-view-and-pure-fn.md (view + TS 純粋関数)
...
--   - 集約 view が ADR 0024 の「status='done' / task_category not null / 外れ値クリップ」
--     をまとめて適用する。
...
-- * 最小サンプル数の判定は client / TS 関数で行う (ADR 0024 Notes)。
...
comment on view public.task_category_correction_factors is
  '見積もり補正倍率 (Phase 3, #93)。task_category 別に actual_minutes / estimated_minutes の中央値 (factor) と件数 (sample_count) を返す。閾値未満の category もそのまま出るので、呼び出し側で件数を見て補正を適用するか判定する。security_invoker=true なので RLS は呼び出し元の auth.uid() で評価される。詳細は ADR 0024 / 0025。';
