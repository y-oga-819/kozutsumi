...
| skill | いつ invoke するか |
|---|---|
| [`kozutsumi-flow`](.claude/skills/kozutsumi-flow/SKILL.md) | 次の issue を選ぶ / 起票 / close / PR 作成 / 棚卸し / Milestone 操作 / Phase 計画の flow 部分（**flow 専任**） |
...
Phase 計画は両 skill をまたぐ compound 作業（先に `kozutsumi-adr` で ADR 確定 → `kozutsumi-flow` で Milestone + issue 起票）。
