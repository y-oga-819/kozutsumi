...
**実装仕様書 (`docs/specs/*.md`) は作らない**。spec は flow（コードが正）なので、milestone 計画時の設計議論は ADR に、実装単位は issue に分解する。
...
| skill | いつ invoke するか |
|---|---|
| [`kozutsumi-flow`](.claude/skills/kozutsumi-flow/SKILL.md) | 次の issue を選ぶ / 起票 / close / PR 作成 / 棚卸し / Milestone 操作 / milestone 計画の flow 部分（**flow 専任**） |
| [`kozutsumi-adr`](.claude/skills/kozutsumi-adr/SKILL.md) | 設計判断を ADR 化 / ADR の粒度判定 / supersede / milestone 計画の設計判断整理（**stock 専任**） |
...
| [`kozutsumi-html-design-doc`](.claude/skills/kozutsumi-html-design-doc/SKILL.md) | ADR / Issue 群が分散していて全体像が頭に入らない時の **人間用ビュー**を `docs/views/*.html` に書く（milestone 計画 / 設計コンセプト explainer / 機能仕様 explainer）。stock の正は ADR、HTML はそこに収まらない読み下し用。 |

skill を invoke せずに issue 運用や milestone 計画、ADR 起票を行わないこと。運用の一貫性は skill の内容で担保している。
milestone 計画は複数 skill をまたぐ compound 作業（議論を読み下したいなら `kozutsumi-html-design-doc` で HTML view → 確定した判断を `kozutsumi-adr` で ADR 化 → `kozutsumi-flow` で Milestone + issue 起票）。
