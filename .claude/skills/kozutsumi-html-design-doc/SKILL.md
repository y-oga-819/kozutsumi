...
description: kozutsumi の plan / 設計コンセプト / 機能仕様を「人間が上から下に読み下せる単一 HTML ドキュメント」として書く運用ルール。ADR / Issue 群が分散していて全体像が頭に入らない、milestone 計画の議論を 1 枚で見通したい、抽象的な設計コンセプトを比喩や図で説明したい、機能の挙動を TL;DR から実装まで段階的に explain したい、などの作業で必ず参照する。`docs/views/` 配下の HTML を書く / 編集する話題、「設計書を読み下したい」「全体像を 1 枚にまとめたい」「思考の場が欲しい」が出たら invoke する。stock の正は ADR (markdown) であり、HTML はそこに収まらない人間用ビューに限る。
---
...
HTML 内で ADR / Issue / PR を参照する時:

- **ADR**: `docs/adr/NNNN-*.md` への相対リンク（例: `<a href="../adr/0042-...md">ADR-0042</a>`）
- **Issue / PR**: フル URL（`https://github.com/y-oga-819/kozutsumi/issues/N`）
- **コード**: 行番号付きで参照（`src/components/X.tsx:42` のように）
...
- **`kozutsumi-frontend-a11y`** — アプリ本体の React component を書く時の skill。本 skill は **`docs/views/` の静的 HTML 専用** で、a11y skill のスコープ外。
