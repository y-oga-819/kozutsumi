...
description: kozutsumi の plan / 設計コンセプト / 機能仕様を「人間が上から下に読み下せる単一 HTML ドキュメント」として書く運用ルール。ADR / Issue 群が分散していて全体像が頭に入らない、milestone 計画の議論を 1 枚で見通したい、抽象的な設計コンセプトを比喩や図で説明したい、機能の挙動を TL;DR から実装まで段階的に explain したい、などの作業で必ず参照する。`public/views/` 配下の HTML を書く / 編集する話題、「設計書を読み下したい」「全体像を 1 枚にまとめたい」「思考の場が欲しい」「Vercel preview で見たい」が出たら invoke する。stock の正は ADR (markdown) であり、HTML はそこに収まらない人間用ビューに限る。
---
...
**人間が 1 枚を上から下に読み下して頭に入れる用の HTML ビュー**を `public/views/` に置く。

stock = ADR。HTML は **思考の場 + 読み下し用ビュー**。AI ではなく人間のための表現形式。
Next.js の static asset として配信され、**Vercel preview と local dev でのみ閲覧可能**
（production は middleware で 404）。スマホは preview URL からアクセスする。

本 skill は以下の場面で必ず参照する:

- milestone 計画で議論を 1 枚に展開したい（options / trade-offs / risks / open questions）
- 設計コンセプトを比喩・対比・図で explain したい（抽象 → 具体）
- 機能の挙動を TL;DR → 実装 → 注意点 → FAQ で段階的に説明したい
- 既存 ADR 群が分散していて全体像が頭に入らないので人間用ビューを作りたい
- `docs/views/*.html` を新規作成 / 編集する
...
```
public/views/<kebab-title>.html
```

- **配置**: `public/views/` 直下にフラットに置く（用途別サブディレクトリは作らない）
...
1. ブランチに push すると Vercel が preview deploy を作る
2. preview URL の `/views` で全ファイルの一覧（`src/app/views/page.tsx` が `public/views/*.html` を自動 glob）
3. 個別 view は `/views/<kebab>.html`（Next.js の static asset、認証なし）
4. local では `npm run dev` → `http://localhost:3000/views`
5. **production は `/views/*` 全て 404**（`src/middleware.ts` で gate）

preview URL は branch hash 付きで推測困難。共有しなければ事実上 hidden。
...
```
1. ブランチで public/views/<kebab>.html を作る → push → preview で読む
2. 議論が固まる → Decision を ADR に distill (kozutsumi-adr skill)
3. PR をマージ準備するタイミングで public/views/<kebab>.html を削除する commit を追加
4. PR マージ → main には HTML が残らない
```

理由:
- Claude が自動で push するワークフローでは、誤って個人情報 / 内部情報を含めるリスクがある
- 本番には出ないが、main の git history に永続するのは避けたい（GitHub repo が public ならログ全文も公開される）
...
`components.html` に実物のレンダリング例とコピペ用 snippet を並べてある。
**書き始める前に必ず `components.html` を開いて、何が用意されているか確認する**。

主なコンポーネント:

- `eyebrow` — 文書分類タグ（"Milestone plan · Phase 2" など）
- `tldr` — 結論 3〜5 行の枠
- `sec-head` — `<span class="num">01</span>` + 見出し
- `callout` — info / warning / success の補足
- `tag` / `sev` — ステータスバッジ
- `tabs` — コード例の切り替え
- `timeline / milestone` — 縦線 + ドットの段階表示
- `compare-table` — Option 比較表
- `risk-table` — Risk × Severity × Mitigation
- `flow-svg` — データフロー図（実線 / 破線 / 矢印）
- `thread` — 議論引用（avatar + bubble + nested）
- `open-questions` — `<dl>` で Q + decide-with
...
HTML は `/views/...` の Vercel preview 上で開かれるため、`docs/adr/...md` への相対リンクは
preview ドメインで 404 になる。
...
本文中での参照は `<a class="ref">` でクラス付けて識別可能にする。
...
- [ ] CDN / 外部リソース / フレームワーク依存がないか（単一ファイルで開けるか）
- [ ] `components.html` のスタイル語彙から逸脱していないか
- [ ] Related セクションに関連 ADR / Issue を列挙したか
- [ ] **PR マージ前**: `public/views/<kebab>.html` を削除する commit を追加したか (§3 参照)
...
- **`kozutsumi-frontend-a11y`** — アプリ本体の React component を書く時の skill。本 skill は **`public/views/` の静的 HTML 専用** で、a11y skill のスコープ外。
