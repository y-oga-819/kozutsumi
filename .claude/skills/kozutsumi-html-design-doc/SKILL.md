...
local 確認したい時は `python3 -m http.server 8000 --directory docs/views/` などで素直に静的配信。

### URL 推測のしやすさと露出

raw.githack は **public repo 限定**。kozutsumi リポは public 前提なので動く。
URL は `<owner>/<repo>/<branch>/<path>` の形で予測可能だが、
branch 名は Claude Code on the web が生成する hash 付き（`claude/...-W8Ra0`）なので
**branch 名を知らない第三者が辿り着く可能性は実質ゼロ**。
