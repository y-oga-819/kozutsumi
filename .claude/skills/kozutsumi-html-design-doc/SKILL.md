...
配信は **[raw.githack.com](https://raw.githack.com/) 経由** — push するだけで URL で開ける（アプリ本体に組み込まない）。
...
3. push 後、commit message か PR description に **生成された raw.githack URL を 1 行残す**
   （人間が再度開きたい時に探さずに済む）
...
3 用途を念頭に、いま自分が書いているケースに対応する Body 構造を選ぶ。
それぞれ「何を書く時 / 前後ワークフロー / Body 構造」を 1 セクションにまとめてある。

### 4.1 Plan (milestone / 実装計画)

**何を書く時**: milestone を切る前に、議論の全景を 1 枚に展開したい時。
...
5. PR マージ前に HTML を削除する commit を追加する（§2 参照）

**Body 構造**:

```
4a. Options explored          （検討した道、3〜5 案）
4b. Trade-offs                （対比表で軸別比較）
4c. Decision                  （選んだ案 + ADR への参照）
4d. Mechanism                 （SVG フロー図でデータ / 制御の流れ）
4e. Milestones                （タイムラインで段階分割）
```

Decision には **必ず ADR への参照リンクを置く**（HTML 単独で判断を確定させない）。

### 4.2 Concept explainer (設計コンセプト)

**何を書く時**: 「なぜこのアーキにするか」を抽象的に説明したい時。比喩 / 対比 / 図が要る時。
例: 「行動パターン分析の学習ループ」「ユーザー意図と AI 提案の責任境界」のような、ADR 1 本では伝わらない概念。

**Body 構造**:

```
4a. Metaphor                  （比喩 1 つで掴ませる）
4b. Mechanism                 （動く図 / SVG / 対話的デモ）
4c. Compare                   （既存パターンとの対比表）
4d. Where it lives in kozutsumi （実際のどこに現れるか）
```

抽象から具体へ。最初は「これは○○のようなものだ」、最後は「コードのこの関数に当たる」。

### 4.3 Feature explainer (機能仕様)

**何を書く時**: 複雑な機能の挙動（リクエスト経路 / 状態遷移 / 設定方法 / 注意点）を新メンバー視点で explain したい時。コードを読めば分かるが、**最初に全体像を頭に入れたい**人向け。

**Body 構造**:

```
4a. Request path / 状態遷移   （4〜6 ステップで処理経路）
4b. Configuration             （タブで設定例 + 実装例 + 期待値）
4c. Gotchas                   （予期しない挙動の警告）
4d. FAQ                       （<dl> で Q&A）
```
...
## 7. 関連 ADR / Issue の参照ルール

HTML 内で ADR / Issue / PR を参照する時は **GitHub の絶対 URL を使う**
（raw.githack ドメイン上では相対リンクが効かないため）。
...
- [ ] **PR マージ前**: `docs/views/<kebab>.html` を削除する commit を追加したか (§2 参照)
