...
PR は **必ず対応する issue とセットで作る**。issue が無い変更は flow から外れるので、
突発作業も `type:devex` 等で issue を起票してから着手する。
...
- 対応 issue が無い PR は出さない（先に issue を起票する）

### PR 作成前のチェック

- [ ] 「関連 Issue」セクションに `Closes #N` または `Refs #N` を記入した
- [ ] テンプレートの不要セクションは削除した
