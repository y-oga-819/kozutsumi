---
name: kozutsumi-flow
description: kozutsumi リポジトリの GitHub flow（Issue / Milestone / PR）の運用ルール。次にやるタスクを選ぶ、新規 issue を起票する（roadmap / idea / bug）、issue を close する、PR を作成する（テンプレート / `Closes #N` の付け方）、idea backlog を棚卸す、Milestone を作成・close する、milestone 計画の flow 部分（Milestone + issue 一括起票）を実行する、などの作業で必ず参照する。GitHub Issues / Milestones / PR / バグ報告の話題が出たら invoke する。ADR / 設計判断は `kozutsumi-adr` skill が担当。
---
...
- milestone 計画の flow 部分を実行する（**ADR 起票は `kozutsumi-adr` を invoke**）
...
| プロダクト narrative / 検証仮説 | stock | `docs/roadmap.md` | — |
| 未解決の論点 | flow | `docs/open-questions.md` | 本 skill |
| 体験単位（順不同・依存なし）| flow | GitHub **Milestone** | **本 skill** |
...
> **Milestone は体験単位**: 「単体で新しい体験を提供する大きな単位」（例: 「秘書からの相談機能」「朝のスタック自動提案」）。順序や依存関係は持たず、複数 milestone を並行 open にしてよい。優先順は流動的に選ぶ。既存体験の磨き込み（バグ修正・UX 改善のかたまり）は milestone にしない（個別 issue で済ませる）。

### 使わない / 作らないもの

- `docs/specs/*.md` のような長い実装仕様書は作らない。
  spec は flow なので markdown として残す価値が低い（コードが正）。
  milestone 計画時の設計議論は ADR 化（`kozutsumi-adr`）し、実装単位は issue に分解する。
- `phase-N` ラベル: 作らない。
- **Phase 概念**: 廃止。今後の milestone は順序や依存を持たない体験単位で切る。既存の `Phase1：コアUI` / `Phase2：Googleカレンダー連携` / `Phase3：...` milestone は当時の Phase 計画の名残として as-is で残す（grandfather）。
...
1. 進行中の milestone（複数可）の **open** issue を `list_issues` で取得
2. 各 issue の `### 前提・依存` を読み、`Depends on` が全て解決済みのものだけ候補化
3. 候補のうち、`### 見積もり規模` と手空き時間を見て1つ選ぶ
4. milestone 間に順序は無いので、跨いで候補を比較してよい
5. 迷ったら人間に聞く（候補を2-3個並べて判断を仰ぐ）

---

## 6. 新 milestone 起票時の手順

新しい milestone（体験単位）を起こす時は以下を順に行う:

1. **その milestone が「単体で何の新しい体験を提供するか」を 1 文で言語化する**
   - 順序や依存ではなく「ユーザー体験として何が変わるか」で切る
   - 既存体験の磨き込み（バグ修正・UX 改善のかたまり）は milestone にしない（個別 issue で済ませる）
...
3. **Milestone を作成**（GitHub UI で手動、MCP は作成 API 未対応）
   - Title: 体験を端的に表す名前（例: `秘書からの相談機能`）。`PhaseN：` 接頭辞は付けない
   - Description: 1〜2 行で「この milestone を達成すると何が新しく出来るか」のサマリ + 関連 ADR リンク。
     詳細は ADR 側に書いてあるので**コピペしない**（二重管理防止）
4. **Issue を一括起票** (`roadmap.yml` 準拠)
   - `type:roadmap` ラベル + 該当 Milestone を付与
   - `Depends on` を明記して milestone 内の着手順を表現（milestone 間には順序を付けない）
   - 関連する ADR を本文の参照リンクに含める
   - **親 Epic issue は作らない**。issue を束ねる役割は Milestone が担い、横断する narrative は ADR / `open-questions.md` が担うので Epic の独立価値が無い。Phase 1 / 2 では Epic (#22 / #48) を作っていたが、Phase 3 以降は廃止している。

**書かないもの**: `docs/specs/*.md` のような長大な実装仕様書。
spec は flow なので、実装単位の issue と code に分解する。

複数 milestone を同時に open にしてよい。優先順は流動的に決める（§5 の選び方は milestone をまたいで適用する）。
...
- 「進行中の milestone 内でやるべき」→ `type:roadmap` で起票 + 該当 Milestone に割当
- 「動かない / 想定と違う挙動」→ Issue Form `bug.yml` で起票（`type:bug`）
- 「アーキ / 差別化レベルの判断を下した」→ **`kozutsumi-adr` skill を invoke** して ADR 起票

基本は進行中 milestone の完遂を優先する。idea issue は棚卸しで処理する。
バグは緊急度に応じて milestone の合間に差し込む（high なら即着手、low/mid は棚卸しで判断）。
...
| milestone 計画 | — | 新 ADR（大きな判断）| roadmap issue 一括起票 | milestone 作成（体験単位 title）| 本 skill + `kozutsumi-adr` |
...
| milestone 完了 | — | — | — | milestone を close | 本 skill |
| 棚卸し（月1 / milestone 切れ目）| — | — | idea の昇格 / close 提案 | — | 本 skill |

---

## 9. 関連する skill

- **`kozutsumi-adr`** — ADR / 設計判断の運用。milestone 計画では先に invoke して ADR を確定させる。
