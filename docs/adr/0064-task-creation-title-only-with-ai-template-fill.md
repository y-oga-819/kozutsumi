...
- schema 詳細 (項目数 / 必須・任意の境界 / 補完タイミング / ユーザー手動入力との競合解決) は M-β 設計時に確定する。本 ADR では「title 必須 / メモ書き推奨 / 残りは AI 補完」までを決める → schema の項目は [ADR-0066](./0066-decompose-completion-criteria-deliverable-done-first-step.md)、補完タイミング / 競合解決は [ADR-0067](./0067-ai-decompose-opt-out-auto-firing.md) で確定。後追い補完は本 ADR のとおり auto 固定 (Alternatives の「AI 補完を default オフ … 不採用」は ADR-0067 でも維持)。競合は AI が未補完フィールドのみ書く + AI 実行中ロックで回避する。
