...
- 削除子に紐づく既存の `task_started` / `task_paused` / `task_completed` 等の action_log は **そのまま残す** (ADR 0001 の方針)。ただし `action_logs.task_id` 列は schema 定義 (`on delete set null`) で target 削除時に **自動的に null へ書き換わる**。`metadata.task_id` (= 一次の真実) は保持されるので、Phase 4 集計側では「`column.task_id` が null かつ `metadata.task_id` が ある」行を「再分解で消えた子のログ」として識別し、`task_child_resplit.resplit_target_snapshot` の `id` と join して属性を引く想定 (ADR 0001 + initial_schema.sql の FK ON DELETE SET NULL と整合)
