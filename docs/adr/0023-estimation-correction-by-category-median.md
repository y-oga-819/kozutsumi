# ADR 0023: 見積もり補正は task_category 別の中央値で算出する
...
- **実装が単純**。SQL の `percentile_cont(0.5)` と TS 純粋関数の median 実装で同じロジックが書ける（計算場所の判断は ADR 0024）。
...
- 計算場所（Supabase view / Route Handler / client）の判断は別 ADR (0024)。
